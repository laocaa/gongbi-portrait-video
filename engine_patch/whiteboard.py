...
CANVAS_BG: tuple[int, int, int] = (255, 255, 255)  # paper color; set by render_gongbi
...
def _line_art_ink_mask(line_art: Image.Image, size: tuple[int, int], threshold: int = DEFAULT_LINE_ART_SNAP_THRESHOLD) -> Image.Image:
    ...
    # full-ink reveal: the pen reveals the complete artwork (thick washes
    # included), not just a 1px skeleton. Fill strokes cover the rest.
...
def _prepare_timeline(strokes: list[Stroke], draw_frames: int) -> list[TimelineStroke]:
    ...
    if strokes_per_frame >= 1.4 or pixels_per_frame >= 100:
        pause_ratio = 0.008
    elif strokes_per_frame >= 0.55 or pixels_per_frame >= 60:
        pause_ratio = 0.018
    else:
        pause_ratio = 0.04
...
def render_scene(
    ...
    reveal_width = max(effective_line_thickness * 10 + 6, int(round(estimated_line_width * 6)), 30)
    snap_ink_mask = (
    ...
    canvas = Image.new("RGB", (resolution[0] * aa_scale, resolution[1] * aa_scale), CANVAS_BG)
    ...
    completion_frames = min(max(60, int(round(fps * 2.5))), max(1, draw_frames // 3))
        ...
        for frame_index in range(total_frames):
            if frame_index < draw_frames:
                ...
                if show_cursor and fill_cursor is not None:
                    ...
                    lift = ease_in_out_sine(progress)
                    lift_x = cursor[0] + lift * resolution[0] * 0.58
                    lift_y = cursor[1] + lift * resolution[1] * 0.58
                    if hand_cursor is not None:
                        frame = _paste_hand_cursor(frame, lift_x, lift_y, hand_cursor)
                    elif hand_style != "none":
                        _draw_cursor(frame, lift_x, lift_y, angle + 0.12)
